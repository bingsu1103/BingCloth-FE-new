...
const CartDrawer = ({ open, setOpen }) => {
            ...
            try {
                if (open && isAuthenticated && user && user._id) {
                    ...
                    if (res?.EC === 0 && Array.isArray(res.data)) {
                        setCartItems(res.data.find(order => order.status === 'none').items);
                    } else {
                        setCartItems([]);
                    }
                }
    ...
    const handleDeleteItem = async (id) => {
        try {
            ...
            if (!res || res.EC !== 0) {
                return message.warning("Removed from order, but failed to delete item.");
            }
                        ...
                        {cartItems.map(item => (
                            ...
                            >
                                <div className='flex justify-center items-center gap-5'>
                                    <div className='bin' onClick={() => handleDeleteItem(item._id)}>
                                ...
                                <div className="flex flex-col text-xs gap-2 max-w-[40%]">
                                    ...
                                    <div style={{ backgroundColor: item.color }} className={`w-4 h-4 rounded border-2`}></div>
                                </div>
